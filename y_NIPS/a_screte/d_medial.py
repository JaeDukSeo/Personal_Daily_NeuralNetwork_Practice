...
X = np.zeros(shape=(100,256,256,3))
for file_index in range(len(data_array)):
    X[file_index,:,:]   = imresize(imread(data_array[file_index],mode='RGB'),(256,256))
...
X = np.zeros(shape=(70,256,256,1))
for file_index in range(len(data_array)):
    X[file_index,:,:]   = np.expand_dims(imresize(imread(data_array[file_index],mode='F',flatten=True),(256,256)),axis=3)
X[:,:,:,0] = (X[:,:,:,0]-X[:,:,:,0].min(axis=0))/(X[:,:,:,0].max(axis=0)-X[:,:,:,0].min(axis=0))
X = shuffle(X)
s_images = X
...
hide_net1 = CNNLayer(4,4,50,tf_Relu,d_tf_Relu)
...
# make graph
Secret = tf.placeholder(shape=[None,256,256,1],dtype=tf.float32)
Cover = tf.placeholder(shape=[None,256,256,3],dtype=tf.float32)
