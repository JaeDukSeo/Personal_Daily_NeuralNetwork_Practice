...
# ---- Make Objects ----
l1_RCNN = RCNN(4,26,26,1,1,batch_size,tf_tanh,d_tf_tanh)
l2_RCNN = RCNN(4,24,24,1,1,batch_size,tf_Relu,d_tf_Relu)
l3_RCNN = RCNN(4,22,22,1,1,batch_size,tf_tanh,d_tf_tanh)

l4_FNN =  FNN(22*22*1,824)
l5_FNN =  FNN(824,256)
l6_FNN =  FNN(256,10)
...
# --- Auto Train ----
# auto_train = tf.train.AdamOptimizer(learning_rate=0.001).minimize(cost,var_list=l1w+l2w+l3w+l4w+l5w+l6w)
auto_train = tf.train.RMSPropOptimizer(learning_rate=0.001).minimize(cost,var_list=l1w+l2w+l3w+l4w+l5w+l6w)
    ...
    for iter in range(num_epoch):
        ...
        if iter % print_size==0:
            ...
            for test_batch_index in range(0,len(testing_images),batch_size):
                ...
                current_batch_label = testing_lables[test_batch_index:test_batch_index+batch_size,:]

                sess_result = sess.run([cost,accuracy,final_softmax,layer_uodate],feed_dict={x:current_batch,y:current_batch_label})
            ...
            print('Test Current cost: ', avg_cost/len(testing_images),' Current Acc: ', avg_accuracy/len(testing_images),end='\n')
            print("----------\n")
            avg_accuracy = 0
            avg_cost = 0
