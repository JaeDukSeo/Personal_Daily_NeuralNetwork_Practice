...
data_location = "./DRIVE/test/images/"
...
data_location = "./DRIVE/test/1st_manual/"
...
train_images = np.zeros(shape=(20,584,565,1))
train_labels = np.zeros(shape=(20,584,565,1))
test_images = np.zeros(shape=(20,584,565,1))
test_labels = np.zeros(shape=(20,584,565,1))
...
for file_index in range(len(train_data)):
    ...
    test_images[file_index,:,:]    = np.expand_dims(imread(test_data[file_index],mode='F',flatten=True),axis=2)
    test_labels[file_index,:,:]    = np.expand_dims(imread(test_data_gt[file_index],mode='F',flatten=True),axis=2)
...
# --- make graph ---
x = tf.placeholder(shape=[None,584,565,1],dtype=tf.float32)
y = tf.placeholder(shape=[None,584,565,1],dtype=tf.float32)
