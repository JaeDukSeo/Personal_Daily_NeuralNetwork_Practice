...
class FCNN():
    ...
    def feedforward(self,input,stride_row,stride_col,padding,og_input= None,x_input=None,dilated=False,dilation=None):
        ...
        if not x_input==None:
            self.layerA = (self.act(self.layer)**tf_blur(x_input) ) + 1.0*tf_blur(x_input)
        else:
    ...
    for iter in range(num_epoch):
        ...
        if iter%print_size==0:
            ...
            for sample in range(0,408,50):
                ...
                plt.savefig('imagesacce/' +str(iter) +"_"+str(sample) + "_denoised_"+ str(real_PSNR)+ ".png")

                plt.figure()
                plt.imshow(np.squeeze(current_batch_low),cmap='gray')
                plt.savefig('imagesacce/' +str(iter) +"_"+str(sample) + "_noised_"+ str(real_PSNR)+ ".png")

                plt.figure()
                plt.imshow(np.squeeze(current_batch_high),cmap='gray')
                plt.savefig('imagesacce/' + str(iter) +"_"+str(sample) + "_groundt_"+ str(real_PSNR)+ ".png")
                plt.close('all')
            ...
            plt.figure()
            plt.imshow(np.squeeze(image),cmap='gray')
            plt.savefig('imagesacce/' +str(iter) +"_"+str(406) + "_denoised_"+ str(real_PSNR)+ ".png")

            plt.figure()
            plt.imshow(np.squeeze(current_batch_high),cmap='gray')
            plt.savefig('imagesacce/' + str(iter) +"_"+str(406) + "_groundt_"+ str(real_PSNR)+ ".png")
            plt.close('all')
