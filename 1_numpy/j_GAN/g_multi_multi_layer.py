...
G_input = 300
hidden_input,hidden_input2,hidden_input3 = 356,512,686
...
G_W4 = np.random.normal(size=(hidden_input3,784),scale=(1. / np.sqrt(hidden_input3 / 2.)))  *0.002
...
for iter in range(num_epoch):
    ...
    # Func: Gradient
    grad_G_w4_part_1 = ((-1/Dl2_A) * d_log(Dl2).dot(D_W2.T) * (d_ReLu(Dl1))).dot(D_W1.T)
    ...
    if iter == 30:
        learing_rate = learing_rate * 0.001

    # ---- Print to Out put ----
    if iter%10 == 0:
        Z1 = np.random.uniform(-1., 1., size=[4, G_input])
        Z2 = np.random.uniform(-1., 1., size=[4, G_input])
        Z3 = np.random.uniform(-1., 1., size=[4, G_input])
        Z4 = np.random.uniform(-1., 1., size=[4, G_input])
        Z = np.vstack((Z1,Z2,Z3,Z4))
        ...
        plt.savefig('multi7/{}.png'.format(str(iter).zfill(3)+"_Ginput_"+str(G_input)+ \
