...
D_W2 = np.random.normal(size=(hidden_input,1),scale=(1. / np.sqrt(128 / 2.)))     *0.002
# D_b2 = np.random.normal(size=(1),scale=(1. / np.sqrt(1 / 2.)))           *0.002
D_b2 = np.zeros(1)

G_W1 = np.random.normal(size=(G_input,hidden_input),scale=(1. / np.sqrt(100 / 2.)))   *0.002
# G_b1 = np.random.normal(size=(128),scale=(1. / np.sqrt(128 / 2.)))      *0.002
G_b1 = np.zeros(hidden_input)

G_W2 = np.random.normal(size=(hidden_input,784),scale=(1. / np.sqrt(128 / 2.)))  *0.002
...
for iter in range(num_epoch):
    ...
    if iter%1000 == 0:
        ...
        plt.savefig('temp_GPU/{}.png'.format(str(iter).zfill(3)+"_"+str(G_input)+"_"+str(hidden_input)), bbox_inches='tight')
        plt.close(fig)
