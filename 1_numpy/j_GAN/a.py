...
D_w1 = np.random.randn(784,560) * 0.002
D_w2 = np.random.randn(560,1) *  0.002

G_w1 = np.random.randn(100,560) *  0.002
G_w2 = np.random.randn(560,784) *  0.002
...
for iter in range(num_epoch):
    ...
    for batch_size in range(0,len(images_train),100):
        
        # current_image = np.expand_dims(images_train[batch_size,:],axis=0)
        current_image = images_train[batch_size:batch_size+100,:]
        
        curren_generated = np.random.uniform(-1., 1., size=[1, 100])
        ...
        grad_G_part_1 = ((-1 * (1/D_fake) * (1/D_logit_f)/100).dot(D_w2.T) * d_ReLu(D_l1_f)).dot(D_w1.T)
    ...
    if iter % 40 == 0:
        ...
        curren_generated = np.random.uniform(-1., 1., size=[16,100])
