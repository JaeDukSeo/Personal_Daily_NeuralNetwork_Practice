...
def layer_1_update_synthetic_weights():
    layer_1.update_synthetic_weights(layer_1_delta)

def layer_2_update_synthetic_weights():
    layer_2.update_synthetic_weights(layer_2_delta)

def layer_3_update_synthetic_weights():
...
if __name__ == '__main__':
    ...
    for iter in range(iterations):
        ...
        multiprocessing.Process(target=layer_1_update_synthetic_weights).start()
        
        layer_2_delta, layer_3_out = layer_3.forward_and_synthetic_update(layer_2_out)
        multiprocessing.Process(target=layer_2_update_synthetic_weights).start()
        
        layer_3_delta =  layer_3_out - batch_y
        multiprocessing.Process(target=layer_3_update_synthetic_weights).start()
