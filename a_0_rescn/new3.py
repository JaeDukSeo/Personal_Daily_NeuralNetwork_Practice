...
if not os.path.exists('imagesnew3/'):
    os.makedirs('imagesnew3/')
...
class ResCNNLayer():
    ...
    def feed_forward(self,input=None,og_input=None):
        ...
        self.layerRes = self.output = tf.multiply(tf.add(self.layerA,og_input),og_input)
        return self.output
...
l5 = ResCNNLayer(3,9,3,tf_ReLU,d_tf_ReLu)
...
with tf.Session(config=config) as sess:
    ...
    for iter in range(num_epoch):
        ...
        if iter % 10 == 0:
            ...
            plt.savefig('imagesnew3/'+str(iter)+'_og.png')
            plt.imshow(np.squeeze(current_data_noise[1,:,:,:]),cmap='gray')
            plt.savefig('imagesnew3/'+str(iter)+'_noise.png')
            plt.imshow(np.squeeze(temp[1,:,:,:]),cmap='gray')
            plt.savefig('imagesnew3/'+str(iter)+'_denoise.png')
