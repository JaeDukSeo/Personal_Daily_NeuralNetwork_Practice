...
one = np.zeros((119,512,512,3))
two = np.zeros((119,512,512,3))
three = np.zeros((119,512,512,3))

# 1.5 Transfer All of the Data into array
print('===== READING DATA ========')
for file_index in range(len(lstFilesDCM1)):
    one[file_index,:,:]   = imread(lstFilesDCM1[file_index])
    two[file_index,:,:]   = imread(lstFilesDCM2[file_index])
    three[file_index,:,:]   = imread(lstFilesDCM3[file_index])
print('===== Done READING DATA ========')

# 1.75 Make the Training data and make it fir 
training_data = np.vstack((one,two,three[:-2,:,:]))
path = 'dialted_special/'
...
# Make the Netwokr
l1 = ResCNNLayer(13,3,3,tf_log,d_tf_log)
...
l6 = ResCNNLayer(1,3,3,tf_ReLU,d_tf_ReLu)

l1w,l2w,l3w,l4w,l5w,l6w = l1.getw(),l2.getw(),l3.getw(),l4.getw(),l5.getw(),l6.getw()

# Make the graph
x = tf.placeholder(shape=[None,512,512,3],dtype="float")
y = tf.placeholder(shape=[None,512,512,3],dtype="float")
...
layer2 = l2.feed_forward(layer1,layer1)
layer3 = l3.feed_forward(layer2,layer2)

layer4 = l4.feed_forward(layer3,layer3)
layer5 = l5.feed_forward(layer4,layer4)
layer6 = l6.feed_forward(layer5,layer5)

loss = tf.reduce_sum(tf.square(tf.subtract(layer6,y) * 0.5))

grad_6,g6w = l6.backprop(tf.subtract(layer6,y),layer5)
grad_5,g5w = l5.backprop(grad_6,layer4)
grad_4,g4w = l4.backprop(grad_5+decay_propotoin_rate*(grad_6),layer3)

grad_3,g3w = l3.backprop(grad_4+decay_propotoin_rate*(grad_6+grad_5),layer2)
grad_2,g2w = l2.backprop(grad_3+decay_propotoin_rate*(grad_6+grad_5+grad_4),layer1)
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        for current_batch_index in range(0,len(training_data),batch_size):
            ...
            current_batch_noise =  current_batch + 0.3 * current_batch.max() *np.random.randn(current_batch.shape[0],current_batch.shape[1],current_batch.shape[2],current_batch.shape[3])

            current_batch       = float32(current_batch)
            current_batch_noise = float32(current_batch_noise)
        ...
        if iter%10==0:
            ...
            for image_in_one in range(0,len(one)):
                ...
                current_data_noise =   current_batch + 0.3 * current_batch.max() *np.random.randn(current_batch.shape[0],current_batch.shape[1],current_batch.shape[2],current_batch.shape[3])
