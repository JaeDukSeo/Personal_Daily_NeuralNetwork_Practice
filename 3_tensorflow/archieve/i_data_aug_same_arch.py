...
with graph.as_default():
    ...
    w1 = tf.Variable(tf.random_normal([5,5,3,7] ,stddev=0.01 ),name="w1")
    w2 = tf.Variable(tf.random_normal([4,4,7,10] ,stddev=0.01 ),name="w2")
    w3 = tf.Variable(tf.random_normal([3,3,10,15] ,stddev=0.01 ),name="w3")
    w4 = tf.Variable(tf.random_normal([13 * 13  * 15,625],stddev=0.01),name="w4")
    w5 = tf.Variable(tf.random_normal([625,325],stddev=0.01),name="w5")
    w6 = tf.Variable(tf.random_normal([325,3],stddev=0.01),name="w6")

    b1 = tf.Variable(tf.random_normal([7],stddev=0.01),name="b1")
    b2 = tf.Variable(tf.random_normal([10],stddev=0.01),name="b2")
    b3 = tf.Variable(tf.random_normal([15],stddev=0.01),name="b3")

    b4 = tf.Variable(tf.random_normal([625],stddev=0.01),name="b4")
    ...
    layer_2 = tf.nn.conv2d(layer_1_pool,w2,strides=[1,1,1,1],padding="SAME",name="layer_2")
    ...
    layer_3 = tf.nn.conv2d(layer_2_pool,w3,strides=[1,1,1,1],padding="SAME",name="layer_3")
    ...
    layer_4_data = tf.reshape(layer_3_pool,[6,-1],name='layer_4_reshape_Flat')
...
with sess:
    ...
    for iter in range(number_of_epoch):
        ...
        for i in range(0,len(current_x)):
            ...
            temp_data = sess.run([layer_6,loss,optimizer,train_prediction],feed_dict={x:current_x_data,y:current_x_label,keep_prob:0.6,lr:learing_rate_start})
        ...
        for j in range(0,6,6):
            ...
            temp_data = sess.run([layer_6,loss,optimizer,train_prediction],feed_dict={x:current_test,y:current_test_label,keep_prob:1.0,lr:0})
    ...
    for j in range(0,len(test_data)):
        ...
        temp_data = sess.run(train_prediction,feed_dict={x:current_x_data,y:current_x_label,keep_prob:1.0})

        if j % 5 == 0:
            print ("Predicted Index Max : ", np.argmax(temp_data,axis=1))
            print ("GT Index Max : ",np.argmax(current_x_label,axis=1))
            print ("Mactch : " ,( np.argmax(temp_data,axis=1) == np.argmax(current_x_label,axis=1)))
            print ("Mactch count : " , ((np.argmax(temp_data,axis=1) == np.argmax(current_x_label,axis=1)) * 1.0).sum())
            print ('\n\n')

        total_match_gt = np.append(total_match_gt,np.argmax(current_x_label,axis=1))
        total_match_list = np.append(total_match_list,np.argmax(temp_data,axis=1))
        total_match += ((np.argmax(temp_data,axis=1) == np.argmax(current_x_label,axis=1)) * 1.0).sum()
